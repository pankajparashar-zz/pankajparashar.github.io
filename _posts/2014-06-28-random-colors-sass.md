...
<pre class="language-scss"><code>$red: random(256)-1;
$green: random(256)-1;
...
<pre class="language-scss"><code>$alpha: random(100)/100;
...
<pre class="language-scss"><code>// Sass -> Compiled CSS        
rgb(255, 0, 0) -> red
...
<pre class="language-scss"><code>.random-color {
...
<pre class="language-scss"><code>@function color($base, $component) {
...
<pre class="language-scss"><code>$named-colors: 'AliceBlue', 'AntiqueWhite', ..., 'YellowGreen';</code></pre>
...
<pre class="language-scss"><code>@function getRandomColor($format: NULL) {
    @if $format == "hex" {
